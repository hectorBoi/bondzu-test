...
submitContactButton.addEventListener('click', () => {
  const name = dataCleaner(nameContactElem.value);
  const email = dataCleaner(emailContactElem.value);
  const subject = dataCleaner(subjectContactElem.value);
  const message = dataCleaner(messageContactElem.value);
