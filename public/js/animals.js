...
const createButton = (location) => {
  ...
  div.style.color = "red";
  div.setAttribute("onclick", `window.location.href='singleAnimal/${location}'`);
  return div;
}
...
fetch("/animals", {
  ...
  .then(animals => {
    ...
    for (animal of animals) {
      ...
      row.appendChild(col);
      count++;
      if ((count > 0 && count % 2 === 0) || (count === animals.length && animals.length % 2 !== 0)) {
